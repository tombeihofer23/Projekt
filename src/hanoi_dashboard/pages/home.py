...
def update_graph_store(n_clicks: int) -> dict:
    ...
    db_service = SensorDataDbService(DB_CON)
    data: pd.DataFrame = db_service.query_data(24, "6252afcfd7e732001bb6b9f7")
    ...
    for name, group in grouped_data:
        ...
        graph_data[name] = {
            "timestamps": group["timestamp"].dt.strftime("%Y-%m-%dT%H:%M:%S").tolist(),
...
@callback(Output("graph-container", "children"), Input("graph-data-store", "data"))
        ...
        graph_components.append(
            html.Div(
                ...
                children=[  # Optional wrapper div
                    dcc.Graph(
                        figure=fig, id={"type": "dynamic-graph", "index": sensor_key}
